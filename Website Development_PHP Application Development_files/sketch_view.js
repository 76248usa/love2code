	...
	if (!embedPage) {
		...
		$('#saveSketchButton').on('click', function () {
			if ($(this).hasClass('selected')) {
				...
				if (sketch.userID == sessionUser.userID) { //quick save
					$('input[name="parentID"]').val(sketch.parentID);
					$('input[name="saveAsNew"]').val('false');
					quickSave();
				} else { //popup edit panel to fork sketch 
					setupEditSketchPanel();
...
var submitSketch = function (quick) {
	var submitForm = new OPLiveForm({
		...
		onSubmit: function () { //on submit
			$('#saveSketchButton, #codeVersions .btn-primary').button('loading');
...
var setupEditSketchPanel = function (showPanel) {
	...
	if (showPanel !== false) {
		activatePanel($('#saveSketchButton'));
		$('#saveSketchButton').button('submit');
		$('#sideButtons').show();
...
var setupPlus = function () {
	...
	$('#editSketchPanel .plusOnly')
		...
		.addClass('disabled')
		.next('.description')
		.html('you need to be a <a href="/membership/" target="_blank">Plus+ Member</a> to change')
		.css('opacity', 1);
...
var setupComments = function () {
	...
	let commentsEnter = comments
		...
		.classed('summarize', function(d){  return d.body.length>120})
...
var setupEmbed = function () {
	...
	if (sessionUser.membershipType > 0) {
		...
		setEmbedURL();
	} else { //disable the embed options
...
var setupSketchFiles = function () {
	...
	if (sketch.userID == sessionUser.userID && sketch.isPjs > 0 && sketch.visualID > 0) {
		let fileTypes = ".gif,.jpg,.jpeg,.png,.csv,.json,.mov,.ogg,.webm,.mp3,.mp4,.mid,.midi,.wav,.txt,.svg,.otf,.ttf,.obj";
